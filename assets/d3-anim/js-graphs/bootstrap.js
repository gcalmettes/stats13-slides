...
gc.bootstrap.init2 = function() {
  ...
  gc.bootstrap.svg2 = d3.select('#bootstrap2 .placeholder')
    .append('svg')
    .attr('width', gc.bootstrap.WIDTH)
    .attr('height', gc.bootstrap.HEIGHT)
    // .style('background', 'white');

  gc.bootstrap.circles2 = gc.bootstrap.svg2.selectAll("circle")
      .data(d3.range(gc.bootstrap.populationSize).map(function(d,i) {return {x: i%10, y: Math.floor(i/10)}}))
      .enter()
      .append("circle");

  gc.bootstrap.circles2
        .attr("cx", function(d) {return gc.bootstrap.xScale(d.x)})
        .attr("cy", function(d) {return gc.bootstrap.yScale(d.y)})
        .attr("r", gc.bootstrap.pointRadius)
        .style("fill", "lightgray")
...
gc.bootstrap.select_sample_multi_color2 = function(){
  //attribute colors based on number of time point has been 
  //selected
  gc.bootstrap.svg2.selectAll(".selected_point_fill").remove();
  gc.bootstrap.svg2.selectAll(".selected_point_stroke").remove();

  var chosen_circles = []

  for (var i = 0; i < gc.bootstrap.sampleSize; i++){
    index_in_circles = Math.floor(Math.random()*gc.bootstrap.circles2._groups[0].length);
    selected_point = d3.select(gc.bootstrap.circles2._groups[0][index_in_circles])
  ...
  for (var i = 0; i < selected_idx.length; i++){
    ...
    selected_point = d3.select(gc.bootstrap.circles2._groups[0][selected_idx[i].idx])
    ...
    gc.bootstrap.svg2.append("circle")
        .attr("class", "selected_point_stroke")
        .attr("cx", selected_point.attr("cx"))
        .attr("cy", selected_point.attr("cy"))
        .attr("r", gc.bootstrap.pointRadius + (2/3)*gc.bootstrap.pointRadius)
...
gc.bootstrap.reset_color2 = function(){
  //attribute colors based on number of time point has been 
  //selected
  gc.bootstrap.svg2.selectAll(".selected_point_fill").remove();
  gc.bootstrap.svg2.selectAll(".selected_point_stroke").remove();
